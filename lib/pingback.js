...
Pingback.prototype.write = function(data) {
  if (Buffer.isBuffer(data)) {
    this._decoder || (this._decoder = new StringDecoder('utf-8'));
...
Pingback.prototype._handle = function() {
  ...
  // grab the uri's
  var source = this.source = parse(uri[0]),
      target = this.target = parse(uri[1]);
...
Pingback.prototype._pass = function() {
  ...
  request(source, function(err, res) {
    ...
    self.title = title;
    self.excerpt = excerpt.replace(/\s+/g, ' ').slice(0, 300).trim(); // escape ??
...
Pingback.scan = function(text, source, func) {
  ...
  text.replace(/href=([^\s>]+)/gi, function(__, target) { 
    target = parse(target.replace(/['"]/g, '').trim());
    if (!target || !target.host || target.host === source.host) {
      return;
    }
    ping.send(target, source, func);
  });
};
...
var request = function(url, body, func) {
  ...
  req.on('response', function(res) {
    ...
    var end = function() {
      ...
      res.body = body;
      func(null, res);
      //console.log('end', opt.method, url.href);
    };
    res.on('end', end);
...
Pingback.middleware = function(opt) {
  if (typeof opt === 'function') opt = { end: opt };
